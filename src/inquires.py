...
def check_delivery_status(file_name: str):
    ...
    # Explicit copy defends against view-mutation warnings bubbling up
    df = clean_client_names(raw_df.copy())
    ...
    if pd.notna(delivered) and str(delivered).strip():
        print("CURRENT STATUS: DELIVERED")
